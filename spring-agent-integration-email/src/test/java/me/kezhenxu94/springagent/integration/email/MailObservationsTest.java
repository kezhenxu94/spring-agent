...
class MailObservationsTest {
  ...
  void shouldReadAMessage() throws Exception {
    final var observation =
        observations.of(mail("Subject: the build is red", ""), 42L, 7L).orElseThrow();
  ...
  @Test
  @DisplayName("a message nobody vouched for is not read at all")
  void shouldSkipAnUnauthenticatedMessage() throws Exception {
    // Empty rather than an actor-less observation the intake would drop anyway: dropped here it is
    // never parsed and never reaches the funnel, which is what stops a mailbox somebody found the
    // address of from being a way to spend this process's time.
    final var unsigned = message("From: mallory@attacker.example\nSubject: hello\n\nbody\n");

    assertThat(observations.of(unsigned, 42L, 7L)).isEmpty();
  }
  ...
  void shouldMintTheDeliveryIdFromTheUid() throws Exception {
    ...
    final var observation =
        observations
            .of(mail("Message-ID: <forged@attacker.example>\nSubject: x", ""), 42L, 7L)
            .orElseThrow();
  ...
  void shouldCorrelateByThread() throws Exception {
    final var first =
        observations
            .of(mail("Message-ID: <root@apache.org>\nSubject: x", ""), 42L, 7L)
            .orElseThrow();
    final var reply =
        observations
            .of(mail("References: <root@apache.org>\nSubject: Re: x", ""), 42L, 8L)
            .orElseThrow();
    final var later =
        observations
            .of(
  ...
  void shouldScopeTheThreadToItsSender() throws Exception {
    ...
    assertThat(observations.of(impostor, 42L, 8L).orElseThrow().correlationKey())
  ...
  void shouldHashAnOverlongKey() throws Exception {
    ...
    final var observation =
        observations
            .of(mail("References: <" + "x".repeat(400) + "@apache.org>\nSubject: x", ""), 42L, 7L)
            .orElseThrow();
  ...
  void shouldNameASubjectlessMessage() throws Exception {
    final var observation = observations.of(mail("X-Nothing: x", "body"), 42L, 7L).orElseThrow();
  ...
  void shouldCarryTheBody() throws Exception {
    final var observation =
        observations.of(mail("Subject: x", "the disk filled up again"), 42L, 7L).orElseThrow();
