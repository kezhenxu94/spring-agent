 ...
 * for that wait keeps being sent messages it can only throw away, while the replica that could have
 * answered them is passed over; a surface disconnected first hands them straight over. Bean
 * destruction is far too late for that, which is why {@code FeishuLongConnection} closes the
...
class ShutdownOrderTest extends AbstractIntegrationTest {
  ...
  @Test
  void theConnectionIsClosedBeforeTheDrainBegins() {
    ...
    assertThat(acceptingWhenClosed)
        .as("the long connection was never closed on shutdown")
