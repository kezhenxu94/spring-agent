 ...
 * listener writes here, and a websocket subscription is only ever a reader that may or may not be
...
public class RunJournal {
  ...
  /** Told about each event as it lands. One subscribed browser's side of the connection. */
   ...
   * @return false if the run has already finished and everything has been replayed, in which case
   *     the reader has already been detached — there is nothing more coming, whatever the caller
   *     does with its end of the connection
   */
  public boolean attach(final Reader reader, final long from) {
    synchronized (lock) {
      ...
      // append waits for one reader's catch-up; that write is a hand-off to the outbound channel
