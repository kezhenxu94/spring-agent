 ...
 * {@code spring-agent-app-webui}'s {@code SecurityConfigurer} for why that one endpoint is
 * reachable without the role.
 */
@Slf4j
@Component
...
public class WebAuthoritiesMapper implements GrantedAuthoritiesMapper {
  ...
  /**
   * The role a consuming application is expected to require of every request that is not public.
   *
   * <p>Named here rather than in the application because this class is what grants it, and {@code
   * /api/me} reports whether the caller has it. An application that requires some other role of the
   * page's endpoints has to map it itself.
   */
