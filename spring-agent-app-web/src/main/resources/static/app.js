// The page.
//
// The one idea to hold on to: an EventSource here is a *reader* of a run that is happening on the
...
import { applyTranslations, LANGUAGE_NAMES, locale, setLocale, t } from './i18n.js';
...
const state = {
  ...
  stream: null,
  runView: null,
  // Where the current stream got to, so a reconnect resumes rather than repeats. The browser sends
  // Last-Event-ID by itself on its own reconnects; this is for the ones we make.
  lastSeq: 0,
...
/**
 * Attaches to a run. `from` is what this page already has; the browser handles later reconnects on
 * its own with Last-Event-ID, which the server honours the same way.
 */
function attachRun(requestId, from) {
  ...
  const stream = new EventSource(`/api/runs/${requestId}/events?from=${state.lastSeq}`);
  state.stream = stream;

  const on = (name, handler) => stream.addEventListener(name, (event) => {
    // Every event carries its sequence number as the SSE id, so this is also the cursor.
    state.lastSeq = Number(event.lastEventId || state.lastSeq);
    let data;
    try {
    ...
    clearTimeout(settle);
    settle = setTimeout(() => { state.replaying = false; }, 90);
    scrollToEnd();
  });
  ...
  stream.onerror = () => {
    // EventSource reconnects by itself, with Last-Event-ID, and the server replays from there. This
    // is only to say so — the run is unaffected either way.
