...
/**
 * {@link BuiltinModels} as {@code GET {baseUrl}/models} answers it.
 *
 * <p>Without this the only built-in choice anyone can express is "whatever the application's own
 * model is", even where the same gateway serves a dozen others that need no token of the user's
 * own.
 *
 * <p>Best-effort, as the contract says, and this is why: plenty of OpenAI-compatible gateways do
 ...
 */
@Slf4j
public class OpenAiBuiltinModels implements BuiltinModels {
  ...
  static List<String> chatModelsAmong(final List<String> offered) {
    final var chat = offered.stream().filter(OpenAiBuiltinModels::looksLikeChatModel).toList();
