 ...
 * Who may reach what.
 *
 * <p>The same file as {@code spring-agent-app-webui}'s, and it stays the same file: this
 * application adds a chat surface, and that surface reaches Feishu over a long connection it opens
 * itself ({@code com.lark.oapi.ws.Client}) rather than over an HTTP callback. So there is no
 * webhook path to admit, nothing to exempt from CSRF, and no reason for the rules here to differ
 * from the browser-only application's. Anything that changes below belongs in both.
 *
