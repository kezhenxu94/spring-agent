 ...
 * </ul>
 */
public abstract class AbstractToolTranslationsCompleteTest {
   ...
   * predictable ways.
   *
   * <p>The Latin markers are matched as whole uppercase words on purpose. Case-insensitively, "XXX"
   * matches the oc_xxx and om_xxx these descriptions legitimately use as example ids, and
   * "xxx.feishu.cn" in a sample link — which is how the first version of this test reported five
  ...
  private Map<String, ToolTextsInventory.English> english;
  ...
  void inventory() throws Exception {
    english = ToolTextsInventory.englishOf(basePackage(), extraTypes());
    texts = new ModuleToolTexts(bundleBase(), promptDirectory(), Locale.SIMPLIFIED_CHINESE);
  }
