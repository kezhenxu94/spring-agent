 ...
 * is set, which is what makes this indistinguishable from the {@code @ConditionalOnProperty} it
 * replaced.
 *
 * <p>Note for native images: conditions are evaluated during AOT processing, so the backend is
 * fixed when the image is built rather than when it runs. See {@code -PnativeBackends} in {@code
 * springagent.native.gradle}. Classpath presence is the more stable half of this — a property can
 * be changed at runtime and be quietly ignored, whereas a compiled-in binary's classpath cannot.
 *
