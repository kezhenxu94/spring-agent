...
public record KnowledgeScope(String owner, String group, String tenant) {
   ...
   *
   * <p>{@link ScopeTarget} is core's own rather than this record's, shared with memory and with the
   * browser's endpoints: the words are what a tool asks the model for, so one list of them is the
   * point, and the knowledge base is a user of that vocabulary and not its owner.
   */
  public KnowledgeScope owning(final ScopeTarget target) {
