 ...
 *
 * <p>Forked from {@code org.springaicommunity.agent.tools.TodoWriteTool} — same tool name, same
 * description, same {@code Todos} shape, same validation — for one changed line: the tool method
 * takes the list of items directly instead of the {@code Todos} wrapper. Upstream's parameter is
 * named {@code todos} and so is the wrapper's single component, and Spring AI's {@code
 * JsonSchemaGenerator} keys the root schema by parameter name and inlines the parameter type, so
 * the only payload the schema accepted was {@code {"todos": {"todos": [...]}}}. Models emit {@code
 * {"todos": [...]}}, which fails to deserialize, and nothing in the description names the fields,
 * so a run burns turns guessing. Taking the list flattens the schema to what they already send.
 *
 * <p>Everything a caller touches is unchanged — {@link Todos}, {@link Todos.TodoItem} and {@link
 * TodoEventHandler} are byte-identical to upstream's, so a surface rendering a todo list needs
 * nothing but the import. That is deliberate: the fix is <a
 * href="https://github.com/spring-ai-community/spring-ai-agent-utils/pull/74">upstream PR #74</a>,
 * and when it is released this class is deleted and the imports point back at the library.
 *
...
public class TodoWriteTool {
  ...
  public String todoWrite(List<Todos.TodoItem> todos) {
    ...
    return "Todos have been modified successfully. Ensure that you continue to use the todo list to"
        + " track your progress. Please proceed with the current tasks if applicable";
  }

  /**
   * Validates the todo list according to the following rules: - Only one task can be in_progress at
   * a time - Task content and activeForm must not be empty or blank - All tasks must have valid
   * status values
   *
  ...
  private void validateTodos(Todos todos) {
    ...
    long inProgressCount =
        items.stream().filter(item -> item.status() == Todos.Status.in_progress).count();

    if (inProgressCount > 1) {
      throw new IllegalArgumentException(
          "Only ONE task can be in_progress at a time. Found "
              + inProgressCount
              + " in_progress tasks. "
